...
FreshChild --> NamespacesReady --> MountsPrivate --> CgroupAttached
                                                         │
                                                         ▼
ExecReady <-- PrivsLocked <-- CredsDropped <-- HygieneApplied <-- RootTransitioned
  seccomp       caps+nnp       setresuid       rlimits+env+caps     chroot+mounts
```

9 states, 8 transitions. Only `Sandbox<ExecReady>` has an `exec_payload()` method. You literally cannot call it on any other state.

```rust
// This compiles - every step required, in order:
...
let s = s.setup_mounts_and_root(&profile)?;        // RootTransitioned
let s = s.apply_runtime_hygiene(&profile)?;        // HygieneApplied
let s = s.drop_credentials(uid, gid)?;             // CredsDropped
let s = s.lock_privileges()?;                      // PrivsLocked
let s = s.ready_for_exec();                        // ExecReady
s.exec_payload(cmd)?;                              // execvp
...
- **Can't reorder.** `drop_credentials()` only exists on `Sandbox<HygieneApplied>`.
- **Can't go backwards.** Each transition consumes `self` by value.
- **Verified by CI.** 8 compile-fail tests confirm that wrong ordering produces the expected compiler error.
...
|---|---|---|
| `FreshChild` | Proxy child spawned | `Command::pre_exec(unshare)` |
| `NamespacesReady` | PID/mount/net/IPC/UTS namespaces | `unshare(2)` |
| `MountsPrivate` | Mount propagation hardened | `mount(MS_PRIVATE\|MS_REC)` |
| `CgroupAttached` | Joined cgroup for resource limits | cgroup writes |
| `RootTransitioned` | Chroot, /proc, /dev, /sys, bind mounts | `mount(2)`, `chroot(2)` |
| `HygieneApplied` | rlimits, env sanitized, fds closed, bounding caps dropped | `setrlimit`, `close`, `prctl` |
| `CredsDropped` | UID/GID unprivileged, groups cleared | `setresuid(2)`, `setresgid(2)` |
| `PrivsLocked` | Process caps zeroed, NO_NEW_PRIVS set | `capset(2)`, `prctl(2)` |
