...
/// Latency budgets (ms)
const COLD_P50_MS: u64 = 100;
const COLD_P95_MS: u64 = 200;
const WARM_P50_MS: u64 = 150;
const WARM_P95_MS: u64 = 300;
const FULL_P50_MS: u64 = 250;
const FULL_P95_MS: u64 = 500;
...
"#;
    ...
    let passed = stats.p50 < Duration::from_millis(COLD_P50_MS)
        && stats.p95 < Duration::from_millis(COLD_P95_MS);
...
fn benchmark_python_hello_world() -> BenchmarkResult {
    ...
    let passed = stats.p50 < Duration::from_millis(WARM_P50_MS)
        && stats.p95 < Duration::from_millis(WARM_P95_MS);
    let reason = if !passed {
        Some(format!(
...
"#;
    ...
    let passed = stats.p50 < Duration::from_millis(FULL_P50_MS)
        && stats.p95 < Duration::from_millis(FULL_P95_MS);
    let reason = if !passed {
        Some(format!(
