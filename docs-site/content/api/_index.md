...
> **Design Note:** We support all three patterns because each has a legitimate use case. Polling is simplest to implement on the client side. Sync mode is perfect for interactive playgrounds where latency matters. Webhooks are the right answer for batch processing where you submit hundreds of solutions and don't want to hold connections open. Judge0 only supports polling - we think that's leaving value on the table.
