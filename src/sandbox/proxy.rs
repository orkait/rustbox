...
use nix::unistd::{close, dup2, fork, Pid, ForkResult};
...
fn wait_for_payload_and_reap(payload_pid: Pid) -> Result<(Option<i32>, Option<i32>, u32)> {
    ...
    loop {
        match waitpid(Pid::from_raw(-1), Some(WaitPidFlag::WNOHANG)) {
            ...
            Ok(WaitStatus::Exited(_, _) | WaitStatus::Signaled(_, _, _) | WaitStatus::Stopped(_, _) | WaitStatus::Continued(_) | WaitStatus::PtraceEvent(_, _, _) | WaitStatus::PtraceSyscall(_)) => {
...
fn run_proxy(req: &SandboxLaunchRequest) -> Result<i32> {
    ...
    let (stdin_read, stdin_write) = nix::unistd::pipe().map_err(|e| IsolateError::process("pipe(stdin)", e))?;
