...
impl SyscallFilterConfig {
    ...
    pub fn custom_allowlist(syscalls: HashSet<String>, profile_id: String) -> Self {
        Self {
            enabled: true,
            policy: SyscallFilterPolicy::CustomAllowlist {
                syscalls,
...
pub fn install_syscall_filter(config: &SyscallFilterConfig, strict_mode: bool) -> Result<()> {
    ...
    match &config.policy {
        ...
        SyscallFilterPolicy::CustomAllowlist {
            syscalls,
            profile_id,
        } => {
            log::info!(
                "Installing custom syscall allowlist: profile_id={}",
                profile_id
            );
        ...
        SyscallFilterPolicy::ReferenceCatalog {
            architecture,
            catalog_name,
            profile_id,
        } => {
            ...
            log::warn!(
                "Reference catalogs are descriptive only - no guarantees for dynamic runtimes"
            );
...
mod tests {
    ...
    fn test_custom_allowlist_config() {
        ...
        let config =
            SyscallFilterConfig::custom_allowlist(syscalls.clone(), "test-profile".to_string());
    ...
    fn test_install_custom_allowlist() {
        ...
        let config = SyscallFilterConfig::custom_allowlist(syscalls, "test".to_string());
    ...
    fn test_filter_source_display() {
        ...
        assert_eq!(
            SyscallFilterSource::CustomAllowlist.to_string(),
            "custom_allowlist"
        );
        assert_eq!(
            SyscallFilterSource::ReferenceCatalog.to_string(),
            "reference_catalog"
        );
    }
}
