...
pub fn parse_mountinfo() -> Result<Vec<MountEntry>> {
    let content = fs::read_to_string("/proc/self/mountinfo")
        .map_err(|e| IsolateError::Filesystem(format!("Failed to read /proc/self/mountinfo: {}", e)))?;
...
fn is_ephemeral_mount(mount_point: &str) -> bool {
    ...
    // Skip tmpfs mounts
    if mount_point.starts_with("/run") || 
...
pub fn verify_mount_invariance(baseline: &HashSet<String>) -> Result<bool> {
    ...
    if !removed.is_empty() {
        log::warn!("Mount invariance violation: {} mounts removed", removed.len());
...
mod tests {
    ...
    fn test_verify_mount_invariance() {
        ...
        assert!(invariant, "Mount invariance should hold for stable baseline");
    }
