...
impl ProcSysPolicy {
    ...
    fn mount_hardened_proc(&self, proc_path: &Path) -> Result<()> {
        ...
        .map_err(|e| {
            IsolateError::Filesystem(format!("Failed to mount hardened /proc: {}", e))
        })?;
    ...
    fn mount_standard_proc(&self, proc_path: &Path) -> Result<()> {
        ...
        .map_err(|e| {
            IsolateError::Filesystem(format!("Failed to mount standard /proc: {}", e))
        })?;
    ...
    fn mount_sys(&self, sys_path: &Path) -> Result<()> {
        ...
        .map_err(|e| {
            IsolateError::Filesystem(format!("Failed to mount /sys: {}", e))
        })?;
...
mod tests {
    ...
    fn test_sys_policy_description() {
        let policy = ProcSysPolicy::strict_default();
        assert_eq!(policy.get_sys_policy_description(), "disabled (strict default)");
        ...
        assert_eq!(policy.get_sys_policy_description(), "enabled (explicit override)");
    }
}
