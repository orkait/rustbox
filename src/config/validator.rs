...
fn validate_namespaces(config: &IsolateConfig, result: &mut ValidationResult) {
    ...
    if config.strict_mode {
        if !config.enable_pid_namespace {
            result.add_error(
                "enable_pid_namespace must be true in strict mode (judge-v1 requirement)".to_string()
            );
        }
        
        if !config.enable_mount_namespace {
            result.add_error(
                "enable_mount_namespace must be true in strict mode (judge-v1 requirement)".to_string()
            );
        }
    }
    ...
    if config.enable_user_namespace {
        result.add_warning(
            "User namespace is not fully supported in judge-v1 GA (rootful strict is the target)".to_string()
        );
    }
}
...
fn validate_syscall_filtering(config: &IsolateConfig, result: &mut ValidationResult) {
    if config.enable_syscall_filtering {
        result.add_warning(
            "Syscall filtering enabled - no compatibility/correctness/safety guarantees".to_string()
        );
        result.add_warning(
            "Failures under filtering are attributed to filter, not judge infrastructure".to_string()
        );
    }
}
...
mod tests {
    ...
    fn test_zero_memory_limit() {
        ...
        assert!(result.errors.iter().any(|e| e.contains("memory_limit cannot be zero")));
    }
    ...
    fn test_zero_cpu_time_limit() {
        ...
        let result = validate_config(&config).unwrap();
        assert!(!result.is_valid());
        assert!(result.errors.iter().any(|e| e.contains("cpu_time_limit cannot be zero")));
    }
    ...
    fn test_wall_time_less_than_cpu_time() {
        ...
        let result = validate_config(&config).unwrap();
        assert!(!result.is_valid());
        assert!(result.errors.iter().any(|e| e.contains("wall_time_limit") && e.contains("cpu_time_limit")));
    }
    ...
    fn test_syscall_filtering_warning() {
        ...
        let result = validate_config(&config).unwrap();
        assert!(result.warnings.iter().any(|w| w.contains("Syscall filtering enabled")));
    }
